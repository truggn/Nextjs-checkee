...
import ConfirmProduct_Form from "./ConfirmProduct_Form";
//

import { useStyles, useStyles1 } from "./ConfirmProduct.styles";
import {
  checkAccessRight,
...
function ConfirmProduct() {
  ...
  const [accessRightData, setAccessRightData] = React.useState<
    ISystemUserTypePageAccessRight[] | []
  >([]);
  const [checkDetailConfirmProduct, setDetailConfirmProduct] =
    React.useState(false);
  const [checkConfirmProduct, setCheckConfirmProduct] = React.useState(false);
  ...
  const state_product: IProduct[] = useSelector(
    (state: any) => state.CreateProduct.create_product_data
  );
  ...
  let value = {
    ...
    actionName: ["confirmproduct", "detailconfirmproduct"],
  };
    ...
    if (accessRightData) {
      for (let accessRight of accessRightData) {
        if (accessRight["controllerName"] === locationUrl) {
          if (accessRight["actionName"] === "detailconfirmproduct") {
            setDetailConfirmProduct(accessRight["checkAccessRight"]);
          } else if (accessRight["actionName"] === "confirmproduct") {
            setCheckConfirmProduct(accessRight["checkAccessRight"]);
          }
        }
      }
    }
  ...
  const handleClickOpenConfirmDialog = (value: string) => {
    setIdConfirm(value);
  ...
  const handleConfirmProduct = () => {
    dispatch(updateProduct(idConfirm));
    setOpenDeleteDialog(false);
  };
  // ------------------------------------------- handle Delete Dialog ----------------------------------- //
    ...
    <div className="main-content">
      <Card className={classes.root}>
        <div className="row">
          <Grid container>
            <Grid item xs={10}>
              <CardHeader title="Duyệt sản phẩm" />
            </Grid>
          </Grid>
        </div>
      </Card>
                        ...
                        return (
                          <TableRow key={index}>
                            ...
                            >
                              ...
                              {checkConfirmProduct ? (
                                <Tooltip title="Duyệt" placement="top-start">
                                  <IconButton
                                    ...
                                    onClick={() => {
                                      // setParticipant(row);
                                      handleClickOpenConfirmDialog(row._id);
                                    }}
                                  >
                                    <EditIcon />
                                  </IconButton>
      ...
      >
        ...
        <DialogContent>
          <DialogContentText id="alert-dialog-description">
            <span>Bạn có chắc chắn muốn duyệt ?</span>
          </DialogContentText>
        </DialogContent>
        <DialogActions>
          ...
          <Button onClick={handleConfirmProduct} color="secondary" autoFocus>
            Duyệt
          </Button>
        </DialogActions>
      </Dialog>
