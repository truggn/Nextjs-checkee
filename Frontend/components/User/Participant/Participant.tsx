...
import { IListParticipant, loadData, deletedData } from "../../../redux/actions/user/ParticipantActions";
...
function Participant() {
  ...
  console.log(state_selectOranization)
  ...
  const state_participant:IListParticipant[] = useSelector(
    (state: any) => state.ParticipantReducerUser.participantdata
  )
  ...
  useEffect(() => {
    if(state_selectOranization){
      const id =state_selectOranization?state_selectOranization.organizationId:null;
      dispatch(checkAccessRight(value));
      dispatch(loadData(id))
    }
  }, [dispatch,state_selectOranization]);
    ...
    <div className="main-content">
      <Card className={classes.root}>
        <div className="row">
          <Grid container>
            <Grid item xs={10}>
              <CardHeader title={`Quản lý đối tượng tham gia ${state_selectOranization?" ":"(vui lòng chọn đối tác)"}`} />
            </Grid>
