...
import { actionTypes } from "../../actions/user/CreateProductActions";
...
const reducer = (state = initialState, action: any) => {
  return produce(state, (draft) => {
    switch (action.type) {
      case actionTypes.CREATE_PRODUCTUSER_SUCCESS:
        successCreateData(draft, action.payload);
        break;
      case actionTypes.CREATE_PRODUCTUSER_FAILURE:
        failureData(draft, action.payload);
        break;
      case actionTypes.GET_PRODUCTUSER_SUCCESS:
        successGetData(draft, action.payload);
        break;
      case actionTypes.GET_PRODUCTUSER_FAILURE:
        failureData(draft, action.payload);
        break;
